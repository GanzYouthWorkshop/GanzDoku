function parse(src)
{
	...
	{
		console.log("Found an equation! " + p1);
		var p = p1.replace(/(\n| )+/g, '&space;');
	...
	//=========================
	//TODO
	tmp = tmp.replace(/\n(.+?)\n/img, function(m, p1, p2, a, b)
	{
		return "<p>" + p1 + "</p>\n";
	});
	tmp = tmp.replace(/<p><\/p>/img, '');
	// tmp = tmp.replace(/(<br>){2,}/img, '</p><p>');
	// tmp = tmp.replace(/<\/li><br>/ig, '</li>');
	// tmp = tmp.replace(/<br>/img, '<br>\n');
	
	//Cleanup
	//-------
	tmp = tmp.replace(/<p><pre><code><\/p>/img, '<pre><code>');	
	tmp = tmp.replace(/<pre><code>((.|[\n])+?)<\/pre><\/code>/img, function(m, p1, p2, a, b)
	{
		alert(p1);
		return p1.replace(/(<p>)/img, "")
	});	
